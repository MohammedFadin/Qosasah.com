...
class hello extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Qosasah.Hello.View');
		$this->load->model('qosasah_model', null, true);
		$this->lang->load('qosasah');
		
		Template::set_block('sub_nav', 'hello/_sub_nav');
	...
	{
		$this->auth->restrict('Qosasah.Hello.Create');
		...
		if (isset($_POST['save']))
		{
			if ($insert_id = $this->save_qosasah())
			{
				...
				redirect(SITE_AREA .'/hello/qosasah');
			}
			else
			{
	...
	public function edit()
	{
		...
		if (isset($_POST['save']))
		{
			$this->auth->restrict('Qosasah.Hello.Edit');
		...
		{
			$this->auth->restrict('Qosasah.Hello.Delete');
